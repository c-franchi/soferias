...
#   - Agendar Férias (/agendar_ferias) – simulação de agendamento
#   - Cancelar Agendamento (/cancelar_agendamento) – simulação
#   - Alterar Agendamento (/alterar_agendamento) – simulação
#   - Solicitar Aprovação (/solicitar_aprovacao) – simulação
#   - Dashboard (/dashboard, /dashboard_data, /aprovar_pedido, /excluir_agendamentos)
#   - Relatório (/relatorio) – placeholder
...
# app = Flask(__name__)
# app.secret_key = "SUA_CHAVE_SECRETA_AQUI"  # Substitua por uma chave segura para produção
...
#     """
#     Rota: /login
#     - GET: Renderiza o template login.html.
#     - POST: Recebe (usuario, senha) via JSON, valida com AuthManager.
#             Se as credenciais forem válidas, salva o usuário na sessão.
#     """
...
#            Se ocorrer erro, loga a exceção e retorna a mensagem de erro para o cliente.
#     """
#     if request.method == 'GET':
#         if not session.get('logged_in'):
#             return redirect(url_for('login'))
#         return render_template('profile.html', profile_pic_path=session.get('profile_pic'), usuario=session.get('usuario'))
...
#                 file.save(filepath)
#                 profile_pic_url = url_for('static', filename=f'uploads/profile_pics/{filename}')
...
#             if file.filename != "":
#                 planilha_folder = os.path.join(os.getcwd(), 'static', 'uploads', 'planilhas')
...
#     """
#     Rota: /marcar_ferias
#     - GET: Renderiza o template marcar_ferias.html, onde o usuário poderá agendar férias.
#     """
#     if not session.get('logged_in'):
#         return redirect(url_for('login'))
...
# # ------------------------------------------------------------
# # BUSCAR FUNCIONÁRIO (usando setores_funcionarios.db)
# # ------------------------------------------------------------
# @app.route('/buscar_funcionario', methods=['GET'])
# def buscar_funcionario():
#     """
#     Rota: /buscar_funcionario
#     - Recebe a 'chapa' via parâmetro GET.
#     - Consulta o banco setores_funcionarios.db para buscar o funcionário com a chapa informada.
#     - Retorna um JSON com { nome, area } se encontrado; caso contrário, { nome: None }.
#     """
...
# # ------------------------------------------------------------
# # VERIFICAR AGENDAMENTO (usando setores_funcionarios.db)
# # ------------------------------------------------------------
# @app.route('/verificar_agendamento', methods=['GET'])
# def verificar_agendamento():
#     """
#     Rota: /verificar_agendamento
#     - Recebe a 'chapa' como parâmetro GET.
#     - Consulta o banco setores_funcionarios.db para verificar se o funcionário já possui um agendamento.
#     - Retorna { agendado: True, dataFerias, dataRetorno } se houver registro; caso contrário, { agendado: False }.
#     """
#     chapa = request.args.get('chapa')
#     if not chapa:
...
#         cursor.execute('SELECT data_ferias, dias_ferias FROM ferias_agendadas WHERE funcionario_id = ?', (funcionario_id,))
...
#             cursor.close()
#             conn.close()
#             return jsonify(agendado=True, dataFerias=dataFerias, dataRetorno=dataRetorno_dt.strftime('%Y-%m-%d'))
#     cursor.close()
#     conn.close()
#     return jsonify(agendado=False)
...
#     - Verifica se o funcionário já possui agendamento (ele não pode agendar duas vezes).
...
#     # Conecta ao banco de funcionários
#     conn = EmployeeDB.get_connection()
#     cursor = conn.cursor()
...
#     # Verifica se este funcionário já possui um agendamento
#     cursor.execute("SELECT id FROM ferias_agendadas WHERE funcionario_id = ?", (funcionario_id,))
#     if cursor.fetchone():
#         cursor.close()
#         conn.close()
#         return jsonify(success=False, message="Você já possui férias agendadas. Utilize alterar ou cancelar."), 409

#     # Verifica conflitos: Se algum outro funcionário da mesma área tiver agendamento com período sobreposto
#     cursor.execute("""
...
#         conflict_end = (datetime.strptime(conflict_start, '%Y-%m-%d').date() + timedelta(days=conflict_days)).isoformat()
#         cursor.close()
#         conn.close()
#         return jsonify(success=False, conflito=True, nome=conflict_name, dataFerias=conflict_start, dataRetorno=conflict_end, message="Conflito: Outro funcionário do seu setor já está agendado para um período que se sobrepõe. Solicite aprovação."), 409

#     # Se não houver conflito, insere o novo agendamento
#     cursor.execute("INSERT INTO ferias_agendadas (funcionario_id, data_ferias, dias_ferias) VALUES (?, ?, ?)",
...
#     """
#     Rota: /cancelar_agendamento
#     - Recebe JSON com 'chapa'.
#     - Verifica se o funcionário possui um agendamento e o remove.
#     """
#     data = request.get_json()
#     chapa = data.get('chapa')
...
#     funcionario_id = funcionario["id"]
#     cursor.execute("DELETE FROM ferias_agendadas WHERE funcionario_id = ?", (funcionario_id,))
#     conn.commit()
#     cursor.close()
#     conn.close()
...
#     - Recebe JSON com 'chapa', 'dataFerias' e 'diasFerias'.
#     - Verifica se o funcionário possui um agendamento.
...
#         cursor.close()
#         conn.close()
#         return jsonify(success=False, message="Nenhum agendamento encontrado para alteração."), 404

#     # Atualiza o agendamento com os novos dados
#     cursor.execute("UPDATE ferias_agendadas SET data_ferias = ?, dias_ferias = ? WHERE funcionario_id = ?",
...
#         conn = EmployeeDB.get_connection()
#         cursor = conn.cursor()
#         # Insere um novo registro na tabela pedidos_aprovacao
#         cursor.execute('''
...
#         cursor.close()
#         conn.close()
#         return jsonify(success=True, message="Pedido de aprovação enviado ao supervisor.")
#     except Exception as e:
...
#     - Obtém dados do dashboard via DashboardManager e renderiza dashboard.html.
#     """
#     if not session.get('logged_in'):
#         return redirect(url_for('login'))
...
#     """
#     Rota: /dashboard_data
#     - Retorna os dados do dashboard em formato JSON para que dashboard.js monte os gráficos.
...
#     """
#     Rota: /excluir_agendamentos
#     - Chama o DashboardManager para excluir todos os agendamentos.
#     - Retorna success=True se a operação for bem-sucedida.
#     """
#     if not session.get('logged_in'):
#         return jsonify(success=False, message="Não autorizado"), 401
...
#     if not session.get('logged_in'):
#         return redirect(url_for('login'))
#     # Supondo que você já tenha lógica para obter as listas:
#     # - em_ferias: funcionários em período de férias
#     # - agendados: funcionários com férias agendadas
#     # Você deve consultá-las do banco de dados conforme sua implementação.
#     # Aqui, usamos dados dummy:
#     em_ferias = []   # Substitua pela consulta real
#     agendados = []   # Substitua pela consulta real
#     return render_template('relatorio.html', em_ferias=em_ferias, agendados=agendados)


# @app.route('/gerar_pdf')
# def gerar_pdf():
#     # Supondo que você tenha lógica para obter as listas:
#     em_ferias = []  # Substitua pela consulta real
#     agendados = []  # Substitua pela consulta real
#     html = render_template('relatorio_pdf.html', em_ferias=em_ferias, agendados=agendados)
#     pdf = html_to_pdf(html)
#     if pdf:
#         response = make_response(pdf)
#         response.headers['Content-Type'] = 'application/pdf'
#         response.headers['Content-Disposition'] = 'inline; filename=relatorio_ferias.pdf'
#         return response
#     else:
...
app = Flask(__name__)
app.secret_key = (
    "SUA_CHAVE_SECRETA_AQUI"  # Substitua por uma chave segura para produção
)
